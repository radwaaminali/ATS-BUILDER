...
const initialData: CVData = {
  ...
  aboutMe: '',
  jobTarget: '',
  education: [{ id: 'edu-1', degree: '', major: '', institution: '', graduationYear: '', grade: '' }],
  experience: [{ id: 'exp-1', company: '', title: '', period: '', achievements: '' }],
  certifications: [{ id: 'cert-1', name: '', date: '' }],
  customSections: [],
...
const App: React.FC = () => {
  ...
  const handleExportPDF = async () => {
    ...
    // الحل الجذري للـ PDF الفارغ: نسخ العنصر لخارج الـ Scroll Area
    const tempContainer = document.createElement('div');
    tempContainer.style.position = 'absolute';
    tempContainer.style.left = '-9999px';
    tempContainer.style.top = '0';
    tempContainer.style.width = '210mm'; // عرض A4 ثابت
    ...
    const opt = {
      ...
      html2canvas: { 
        ...
        windowWidth: 794 // عرض A4 بالبكسل (96dpi)
      },
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          <div className={`no-print w-full lg:w-1/2 space-y-6 ${activeTab === 'preview' ? 'hidden lg:block' : ''}`}>
            <div className="flex p-1 bg-slate-200 rounded-2xl mb-6 gap-1 overflow-x-auto shadow-inner">
              <button onClick={() => setActiveTab('refresh')} className={`shrink-0 flex-1 py-3 px-6 rounded-xl font-bold text-[11px] transition-all ${activeTab === 'refresh' ? 'bg-white shadow text-rose-600' : 'text-slate-500'}`}>🔄 تحديث ذكي</button>
              <button onClick={() => setActiveTab('edit')} className={`shrink-0 flex-1 py-3 px-6 rounded-xl font-bold text-[11px] transition-all ${activeTab === 'edit' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>تعديل السيرة</button>
              <button onClick={() => setActiveTab('linkedin')} className={`shrink-0 flex-1 py-3 px-6 rounded-xl font-bold text-[11px] transition-all ${activeTab === 'linkedin' ? 'bg-white shadow text-blue-600' : 'text-slate-500'}`}>LinkedIn</button>
              <button onClick={() => setActiveTab('settings')} className={`shrink-0 flex-1 py-3 px-6 rounded-xl font-bold text-[11px] transition-all ${activeTab === 'settings' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>التنسيق</button>
            </div>
          ...
          <div className={`w-full lg:w-1/2 ${['edit', 'settings', 'linkedin', 'refresh'].includes(activeTab) ? 'hidden lg:block' : ''}`}>
